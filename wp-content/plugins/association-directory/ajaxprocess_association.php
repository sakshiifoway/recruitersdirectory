...
if($flag == "paging")
{
	...
	if($get_total_rows>0)
	{
		foreach($faq as $row){
			...
			$edit_url = admin_url('admin.php?page=add_update_association&id=' . $id);
			$delete_url = admin_url('admin.php?page=manage_associations&did=' . $id);
			
			$outputTable .= '
			<tr>
				<td class="manage-column ss-list-width">'.$association.'</td>
				<td class="manage-column ss-list-width">'.$specialty.'</td>
				<td class="manage-column ss-list-width">'.$location.'</td>
				<td>
					<a href="'.$edit_url.'"><img src="'.get_template_directory_uri().'/assets/images/edit.png" alt="Edit"></a>
                    <a onclick="deleteAssociation(\''.$delete_url.'\');" style="cursor:pointer;"><img src="'.get_template_directory_uri().'/assets/images/delete.png" alt="Delete"></a>
				</td>
         	</tr>
			';
		}
