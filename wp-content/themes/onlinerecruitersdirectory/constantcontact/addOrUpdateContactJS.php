...
if (isset($_POST['email']) && strlen($_POST['email']) > 1) {
    ...
    try {
        ...
        echo '</pre></div>';
        die();*/
		echo "<script language='javascript'>window.location.href='http://www.onlinerecruitersdirectory.com/newsletterthankyoujs.php';</script>";	
    }
	
	if (isset($returnContact)) {
	echo "<script language='javascript'>window.location.href='http://www.onlinerecruitersdirectory.com/newsletterthankyoujs.php?opt=success1';</script>";		
...
<div class="dign_upinpt">
	<form class="form-horizontal" name="submitContact" id="submitContact" method="POST" action="constantcontact/addOrUpdateContactJS.php">
    	<?php if($_REQUEST['opt']=='success1'){echo '<div style="color:#3AA542;text-align:center;font-size:10px;margin:0px -100px 0px 0px;">Contact Added Successfully.</div>';} else {?>
        ...
        <div class="control-group" style="margin:3px -10px 0px 0px; float:right;">
            <div class="controls">
                <input type="email" id="email" name="email" placeholder="Email Address" style="width:100px; margin-right:7px;">
            </div>
        </div>
        <div class="control-group" style="margin:3px -10px 0px 0px; float:right;">
            <div class="controls">
                <input type="text" id="first_name" name="first_name" placeholder="First Name" style="width:100px; margin-right:7px;">
            </div>
        </div>
        ...
        <div class="join_now">
        	<input type="submit" value="" class="join_btn" style="background:url('images/join_btn.png') no-repeat scroll left top rgba(0, 0, 0, 0); width:101% !important; margin:17px -133px 0px 0px;" onClick="javascript:overlay();"/>
        </div>
